...
def Random_ex(gene_nums,driver_gene_nums):
    rows = gene_nums
    cols = len(driver_gene_nums)
    # 生成一個全為0的DataFrame
    df = pd.DataFrame(np.zeros((rows, cols), dtype=int), columns=[f'random{driver_gene_nums[k]}' for k in range(0, cols)])

    # 在隨機位置放置指定數量的1
    for col, num_ones in enumerate(driver_gene_nums):
        random_rows = np.random.choice(rows, size=num_ones, replace=False)
        df.iloc[random_rows, col] = 1
    return df   
...
def main():
    ...
    raw_label = Random_ex(20530,[300,400,600,700])
    ...
    for dl in range(0,raw_label.shape[1]):    
        ...
        all_pre_sorce.to_excel('pre_sorce_'+ benchmark_name[dl]+'-all_withoutv'+'.xlsx',header=1,index=0)
        all_Gold_standard.to_excel('Gold_standard_'+ benchmark_name[dl]+'-all_withoutv'+ '.xlsx',header=1,index=0)
