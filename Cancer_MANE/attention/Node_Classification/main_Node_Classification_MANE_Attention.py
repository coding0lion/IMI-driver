...
from args_parser_Node_Classification_MANE_Attention import get_parser
from collections import OrderedDict



class MANEAttention(nn.Module):  # ready for cluster, no cache cleaning or loop shortening
    def __init__(self, n_classes, params, len_common_nodes, embed_freq, batch_size, negative_sampling_size=10):
        ...
        self.classifier_w = nn.Parameter(
            torch.randn(self.embedding_dim * self.num_net, self.n_classes, requires_grad=True))
    ...
    def supervision(self, node_idx, label, hyp_s):

        nodes_idx = torch.LongTensor(node_idx).to(self.device)  # JUST ONE NODE
        tan_h = nn.Tanh()
        ...
        for i_tensor in range(1, self.num_net):
            self.final_emb = (torch.cat((self.final_emb, torch.bmm(self.node_embeddings[i_tensor].weight[
                                                                       ...
                                                                       for i_att in range(self.num_net))).unsqueeze(
                2))), 1))

        self.final_emb = tan_h(self.final_emb.squeeze()[Variable(all_idx)])

        label = torch.Tensor(label).to(self.device)  # no need for one sample
        # loss function:

        loss_labels = torch.sum(torch.log(1 + torch.exp((torch.matmul(self.final_emb[Variable(nodes_idx)],
                                                                      self.classifier_w.to(
                                                                          self.device)) + self.classifier_b.to(
            self.device)).neg() * label)), dim=1).mean()
...
def read_supervision_node_labels(current_path, splits, repeat):
    """
    Attention version, train indices and train labels are read for node prediction task.
    :param current_path:  train indices and train labels of nodes for each fold.
    ...
    return train_splits, train_label_splits
...
def main():
    ...
    G = read_graphs(params.input_graphs + params.dataset+str(params.cancer)+'/', params.nviews)
    ...
    train_splits, train_label_splits = read_supervision_node_labels(params.att_node_label_path+params.cancer+'/' ,
                                                                    params.nsplits,
                                                                    repeat)
    ...
    while epo <= params.epochs - 1:
        ...
        loss2 = model.supervision(train_splits[split_indice], train_label_splits[split_indice], params.gamma)
        loss2.backward()
        ...
        optimizer.zero_grad()  # put here
        loss3 = model.supervision(train_splits[split_indice], train_label_splits[split_indice], params.gamma)
        loss3.backward()
